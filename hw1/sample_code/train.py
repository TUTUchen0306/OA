...
from utils import MNIST_dataset, MNIST_logistic, MNIST_nn_one_layer, AverageCalculator, accuracy, plot_train_stats, plot_optimal_loss
...
def train_SVRG_one_iter(model_k, model_snapshot, optimizer_inner, optimizer_snapshot, train_loader, snapshot_loader, loss_fn):
    ...
    for images, labels in snapshot_loader:
        ...
        snapshot_loss = loss_fn(yhat, labels) / n
    ...
    for images, labels in train_loader:
        ...
        yhat_snapshot = model_snapshot(images)
        loss_snapshot = loss_fn(yhat_snapshot, labels)
...
if __name__ == "__main__":
    ...
    # if args.nn_model == "MNIST_logistic":
    ...
    plot_optimal_loss(train_loss_all, log_dir)
